@extends('layouts.app')
@section('title', 'Tambah Data Peserta KB')

@section('main')
<div class="main-content">
    <section class="section">
        <div class="section-header"><h1>Tambah Data Peserta KB Baru</h1></div>
        <div class="section-body">
            <div class="card">
                <form action="{{ route('peserta-kb.store') }}" method="POST">
                    @csrf
                    <div class="card-body">
                        ...
                        <div class="form-group">
                            ...
                            <input type="date" name="tanggal_pelayanan" class="form-control @error('tanggal_pelayanan') is-invalid @enderror" value="{{ old('tanggal_pelayanan', date('Y-m-d')) }}" required>
                            @error('tanggal_pelayanan') <div class="invalid-feedback">{{ $message }}</div> @enderror
                        </div>
                        <div class="form-group">
                        ...
                        <div class="form-group">
                            ...
                            <select name="jenis_kontrasepsi" class="form-control @error('jenis_kontrasepsi') is-invalid @enderror" required>
                             ...
                             @error('jenis_kontrasepsi') <div class="invalid-feedback">{{ $message }}</div> @enderror
                        </div>
                        <div class="form-group">
                            <label>Jalur Layanan</label>
                            <select name="jalur_layanan" class="form-control @error('jalur_layanan') is-invalid @enderror" required>
                                <option value="">-- Pilih Jalur --</option>
                                <option value="UMUM" {{ old('jalur_layanan') == 'UMUM' ? 'selected' : '' }}>UMUM</option>
                                <option value="BPJS/K" {{ old('jalur_layanan') == 'BPJS/K' ? 'selected' : '' }}>BPJS/K</option>
                                <option value="PASCA SALIN" {{ old('jalur_layanan') == 'PASCA SALIN' ? 'selected' : '' }}>PASCA SALIN</option>
                            </select>
                            @error('jalur_layanan') <div class="invalid-feedback">{{ $message }}</div> @enderror
                        </div>
                    </div>
